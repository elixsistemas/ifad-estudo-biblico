...
// ---------- helpers de fetch ----------
async function getJsonOrError(res){ const t=await res.text(); try{const j=t?JSON.parse(t):{}; if(!res.ok) throw new Error(j?.msg||j?.message||`HTTP ${res.status}`); return j;}catch{ if(!res.ok) throw new Error(t||`HTTP ${res.status}`); return {};}}
async function fetchChapter({version,abbrev,chapter}){ const url=`https://www.abibliadigital.com.br/api/verses/${version}/${abbrev}/${chapter}`; const res=await fetch(url,{headers:{Authorization:`Bearer ${TOKEN}`,Accept:"application/json"}}); return getJsonOrError(res);}
async function fetchVerse({version,abbrev,chapter,number}){ const url=`https://www.abibliadigital.com.br/api/verses/${version}/${abbrev}/${chapter}/${number}`; const res=await fetch(url,{headers:{Authorization:`Bearer ${TOKEN}`,Accept:"application/json"}}); return getJsonOrError(res);}
async function fallback({abbrev,chapter,number}){ const ref=number?`${abbrev}+${chapter}:${number}`:`${abbrev}+${chapter}`; let r=await fetch(`https://bible-api.com/${encodeURIComponent(ref)}?translation=almeida`); const d=await r.json(); if(d?.text) return {book:{name:"Fallback"},chapter:`${chapter}`,number,text:d.text}; if(Array.isArray(d?.verses)) return {book:{name:"Fallback"},chapter:{number:chapter},verses:d.verses.map(v=>({number:v.verse,text:v.text}))}; throw new Error("Fallback falhou"); }
...
// ---------- URL helpers ----------
function readParams(){ if(typeof window==="undefined") return {}; const sp=new URLSearchParams(window.location.search); return {version:(sp.get("version")||"acf"), book:(sp.get("book")||"jo").toLowerCase(), chapter:Number(sp.get("chapter")||3), verse:sp.get("verse")?Number(sp.get("verse")):"", day:sp.get("day")||"", i:sp.get("i")?Number(sp.get("i")):null}; }
function syncUrl({version,book,chapter,verse,day,i}){ if(typeof window==="undefined") return; const p=new URLSearchParams({version,book,chapter:String(chapter)}); if(verse) p.set("verse",String(verse)); if(day) p.set("day",day); if(i!==null && i!==undefined) p.set("i",String(i)); window.history.replaceState({}, "", `/app/reader?${p.toString()}`); }
...
export default function Reader(){
  ...
  React.useEffect(()=>{ syncUrl({version,book:abbrev,chapter,verse:number,day,i:idx}); /* mount sync */ },[]);

  async function fetchAndSet(v=version,b=abbrev,c=chapter,n=number){
    setLoading(true); setError(""); setData(null);
    try { const payload = n? await fetchVerse({version:v,abbrev:b,chapter:c,number:n}) : await fetchChapter({version:v,abbrev:b,chapter:c}); setData(payload); }
    catch(err){ const msg=String(err?.message||err); if(/401|403|token|authorized/i.test(msg)){ try{ const p2=await fallback({abbrev:b,chapter:c,number:n}); setData(p2); setError("Usando fonte pública (fallback)."); } catch(e2){ setError(`Falhou (ABD ${msg}) e fallback: ${e2.message}`);} } else setError(msg);}
